...
public class EnvInfoDao {
    ...
    public List<DsgcEnvInfoCfgBean> queryApiEnvCfgList(CommonReqBean q) {
        ...
        if ("mysql".equals(dbType)) {
            //str = "SELECT de.*,IF(@temp=de.ENV_TYPE, @rank:=@rank+1, @rank:=1) row_number,@temp:=de.ENV_TYPE FROM DSGC_ENV_INFO_CFG de  ";
            str="SELECT de.*,@rownum:=@rownum+1 as row_num FROM (SELECT @rownum:=0) r,DSGC_ENV_INFO_CFG de  ";
        }
     ...
     * @return
     */
    public DSGCEnvServerCfg getCfgByServerName(String envCode, String serverName) {
        return this.sw.buildQuery()
                .eq("envCode", envCode)
                .eq("serverName", serverName)
                .doQueryFirst(DSGCEnvServerCfg.class);
    }
     ...
     * @return
     */
    public DSGCEnvMachineCfg getCfgByMachineName(String envCode, String machineName) {
        return this.sw.buildQuery()
                .eq("envCode", envCode)
                .eq("machineName", machineName)
                .doQueryFirst(DSGCEnvMachineCfg.class);
    }

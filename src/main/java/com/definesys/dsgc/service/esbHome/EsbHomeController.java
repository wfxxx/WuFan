...
//获取ESB首页卡片视图数据
    ...
    @PostMapping("/queryFailedInstancesList")
    public Response queryFailedInstancesList(@RequestParam(value = "pageSize", required = false, defaultValue = "10") int pageSize,
                                             @RequestParam(value = "pageIndex", required = false, defaultValue = "1") int pageIndex){
        try {
            PageQueryResult<FailedInstanceResVO> result =  esbHomeService.queryFailedInstancesList(pageIndex,pageSize);
            return Response.ok().setData(result);
        }catch (MpaasBusinessException e){
            e.printStackTrace();
            return Response.error("出现内部错误，请联系管理员");
        }
    }

...
public class EsbHomeDao {
    ...
    //TODO
    public List<Map<String, Object>> queryBusSortData(String limitTime) {
        MpaasQuery mq = sw.buildQuery();
        String sql = null;
        if ("mysql".equals(dbType)) {
            switch (limitTime) {
                case "year":
                    sql = " select sum(t.bus_total_times) as total,t.month month from rp_serv_month t where t.year=LPAD(DATE_FORMAT(CURRENT_TIMESTAMP,'%Y'),4,0) group  by t.month order by t.month desc";
                    break;
                case "month":
            ...
            switch (limitTime) {
                case "year":
                    sql = "select sum(t.bus_total_times) as total,t.month month from rp_serv_month t " +
                            " where to_date(t.year,'yyyy')=to_date(to_char(sysdate,'yyyy'),'yyyy') group  by t.month order by t.month desc";
                    break;
                case "month":
                    sql = "select sum(t.bus_total_times) as total,t.day day from rp_serv_day t " +
                            " where to_date(t.year||'-'||t.month,'yyyy-mm')=to_date(to_char(sysdate,'yyyy-mm'),'yyyy-mm') group  by t.day order by t.day desc";
                    break;
                case "week":
                    sql = "select sum(t.bus_total_times) as total,t.day day from rp_serv_day t" +

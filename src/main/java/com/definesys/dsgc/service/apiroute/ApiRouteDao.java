...
public class ApiRouteDao {
    ...
    public PageQueryResult queryApiRouteList(CommonReqBean param, int pageSize, int pageIndex, String userRole, List<String> sysCodeList) {
        ...
        if ("mysql".equals(dbType)) {
            sqlStr = new StringBuffer("SELECT t1.*,t2.env_code FROM ( SELECT dr.*, dse.sys_name appName FROM dag_routes dr,dsgc_system_entities dse WHERE " +
                    " dr.app_code = dse.sys_code ) t1,( SELECT *  FROM (SELECT t.*,@row_number := CASE WHEN @customer_no = t.sour_code THEN " +

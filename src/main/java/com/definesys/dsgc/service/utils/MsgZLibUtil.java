...
//		try {
			...
			// 截取前4位byte(存储了msg对应的byte数组长度的byte)
			//lenByte = ByteUtil.subbytes(xmlByte, 0, 4);
			// 获取长度byte存储的值(msg对应的byte的长度)
			//int len = ByteUtil.byteToInt(lenByte);
			//System.out.println("长度存储byte数组存储的value：" + len);

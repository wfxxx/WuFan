...
public class ApiBsDao {
    ...
    public PageQueryResult queryApiBsList(CommonReqBean param, int pageSize, int pageIndex, String userRole,List<String> sysCodeList){
        ...
        if("oracle".equals(dbType)){
             sqlStr = new StringBuffer("  select * from (                               \n" +
                     "    select t.*,row_number() over(partition by t.bs_code order by 1) as rowrid from \n" +
                     "    (    " +
                     "         select  db.*,dse.sys_name appName,stat.env_code from dag_bs db  \n" +
                     "         left join dsgc_system_entities dse on   db.app_code = dse.sys_code\n" +
                     "         left join  dag_code_version ver on db.bs_code =ver.sour_code\n" +
    ...
    if ("mysql".equals(dbType)){
        sqlStr = new StringBuffer("SELECT t1.*,t2.env_code FROM (" +
                "SELECT db.*,dse.sys_name appName FROM dag_bs db,dsgc_system_entities dse WHERE db.app_code = dse.sys_code ) t1," +

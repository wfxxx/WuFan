...
@SQLQuery(value = {
        ...
        @SQL(view = "bus_allInfo_view", sql = "  select cf.*, mc.server_count ,sec.node_count from dsgc_env_info_cfg cf,(select count(mc.demc_id) server_count, mc.env_code from dsgc_env_machine_cfg mc, dsgc_env_info_cfg c where mc.env_code = c.env_code group by mc.env_code) mc, (select count(sc.desc_id) node_count, sc.env_code from dsgc_env_server_cfg sc, dsgc_env_info_cfg c where sc.env_code = c.env_code group by sc.env_code) sec where cf.env_code = mc.env_code(+) and cf.env_code = sec.env_code(+) order by cf.env_seq asc  ")
})

...
public class DSGCServiceStatisticsReportDao {
    ...
    public List<Map<String, Object>> getServMonthRunData(RpServMonth rpServMonth){
        ...
        if ("mysql".equals(dbType)){
            sql = "SELECT sum(month.total_times) as sum_total,round(avg(month.avg_cost),2) as avg_cost,sum(month.total_times_f) as sum_total_f, date_format(str_to_date(concat(avg(month.YEAR),concat('-',avg(month.MONTH))),'%Y-%m'),'%Y-%m') as dates FROM RP_SERV_MONTH month " +
                    "  WHERE YEAR = #year AND MONTH >= #startMonth AND MONTH <= #endMonth AND SERV_NO = #servNo group by YEAR,MONTH order by month.MONTH";
        }
        return sw
                .buildQuery()

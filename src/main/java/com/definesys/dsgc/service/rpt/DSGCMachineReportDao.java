...
public class DSGCMachineReportDao {
    ...
    public List<Map<String,Object>> getMachineMemMsg(Map<String,String> map){
       ...
       if("minute".equals(temp)){
           ...
           if ("mysql".equals(dbType)){
               sql = "SELECT avg(mem.MEM_FREE) as mem_free,avg(mem.MEM_USE) as mem_use,avg(mem.MEM_TOTAL) as mem_total,date_format(LAST_UPDATE_DATE,'%Y-%m-%d %H:%i') as dates FROM DSGC_PC_MEM_INFO_HIS mem WHERE LAST_UPDATE_DATE >= date_format(#startTime,''%Y-%m-%d %H:%i:%s') AND LAST_UPDATE_DATE <= str_to_date(#endTime,''%Y-%m-%d %H:%i:%s') AND SERVER = #serverName group by date_format(LAST_UPDATE_DATE,'%Y-%m-%d %H:%i') order by date_format(LAST_UPDATE_DATE,'%Y-%m-%d %H:%i')";
           }
           return sw
                   .buildQuery()
                   .sql(sql)
    ...
    public List<Map<String,Object>> getMachineDiskMsg(Map<String,String> map){
        ...
        if("hour".equals(temp)){
            ...
            if ("mysql".equals(dbType)){
                sql = "SELECT avg(RTRIM(disk.USAGE,'%')) as disk_usage,date_format(LAST_UPDATE_DATE,'%Y-%m-%d %H') as dates FROM DSGC_PC_DISK_INFO_HIS disk WHERE LAST_UPDATE_DATE >= str_to_date(#startTime,'%Y-%m-%d %H:%i:%s') AND LAST_UPDATE_DATE <= str_to_date(#endTime,'%Y-%m-%d %H:%i:%s') AND SERVER = #serverName group by date_format(LAST_UPDATE_DATE,'%Y-%m-%d %H') order by date_format(LAST_UPDATE_DATE,'%Y-%m-%d %H')";
            }
            return sw
                    .buildQuery()
                    .sql(sql)

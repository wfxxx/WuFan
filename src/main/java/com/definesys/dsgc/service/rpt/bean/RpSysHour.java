...
@ApiModel(value = "系统执行消耗表--小时表",description = "存储系统执行消耗,最小粒度到小时")
@Table("RP_SYS_HOUR")
public class RpSysHour extends MpaasBasePojo {
    @ApiModelProperty(value = "唯一标识",notes = "不能超过30个字符")
    @RowID(type=RowIDType.UUID)
    private String id;

    @ApiModelProperty(value = "接收系统",notes = "不能超过20个字符")
    @Column(value = "RECEIVE")
    ...
    @ApiModelProperty(value = "被调用总次数",notes = "NUMBER类型")
    @Column(value="TOTAL_TIMES")
    private Double totalTimes;

    @ApiModelProperty(value = "被调用成功次数",notes = "NUMBER类型")
    @Column(value = "TOTAL_TIMES_S")
    private Double totalTimesS;

    @ApiModelProperty(value = "被调用失败次数",notes = "NUMBER类型")

...
@ApiModel(value = "接口执行消耗表--月表",description = "存储接口执行消耗,最细粒度到月")
@Table("RP_SERV_MONTH")
@SQLQuery(
        value = {
                @SQL(view = "AllServ_Mouth_Data", sql = "select SERV_NO, sum(TOTAL_TIMES) as TOTAL_TIMES , sum(TOTAL_TIMES*AVG_COST)/sum(TOTAL_TIMES) as AVG_COST, sum(TOTAL_TIMES_S) as TOTAL_TIMES_S ,sum(TOTAL_TIMES_F) as TOTAL_TIMES_F  from RP_SERV_YEAR where  YEAR >= #startYear AND YEAR <= #endYear AND MONTH >= #startMonth AND MONTH <= #endMonth AND TOTAL_TIMES <> '0' group by SERV_NO")
        }
)
public class RpServMonth extends MpaasBasePojo {

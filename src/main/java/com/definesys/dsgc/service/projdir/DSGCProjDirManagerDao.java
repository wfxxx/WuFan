...
public class DSGCProjDirManagerDao {
    ...
    public PageQueryResult<SysProfileDirVO> querySysProfileDirListPage(int pageIndex, int pageSize, SysProfileDirVO sysProfileDirVO) {
        StringBuffer strSql = new StringBuffer("SELECT * FROM (SELECT pd.proj_id, pd.proj_name, pd.proj_desc,pd.proj_port, pd.sys_code, ss.sys_name,pd.repo_uri,pd.branch_name,pd.init_status,proj_type,pd.TEXT_ATTRIBUTE1 FROM DSGC_SVCGEN_PROJ_INFO pd LEFT JOIN DSGC_SYSTEM_ENTITIES ss on pd.sys_code = ss.sys_code ) s WHERE 1 = 1 ");

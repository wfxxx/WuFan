...
//}
    public PageQueryResult<DSGCRoleControlVO> queryRoleControl2(DSGCRoleControl roleControl, int pageSize, int pageIndex){
        return sw.buildQuery().sql("select * from (select c.role_id, \n" +
                ...
                "                (case when u.user_name is null then v2.meaning else CONCAT(u.user_description,'-',u.user_name) end) as role_Name,\n" +
                "                v1.meaning as menu_Bar_Name,c.is_see,c.is_edit,c.creation_date,c.menu_bar\n" +

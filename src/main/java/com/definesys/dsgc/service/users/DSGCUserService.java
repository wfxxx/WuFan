...
import com.definesys.dsgc.service.mynty.MyNtyDao;
...
//                this.dsgc_service.saveServiceUser(dsgcServiceUsers.get(i));
    ...
    public String deleteUserById(DSGCUser user) {
        //删除异常订阅
        mndao.deletelSubscribe(user.getUserId());
        //删除通知数据，数据量太大，放弃删除。

        //删除消费者连接
        consumersDao.delConsumerByUserId(user.getUserId());
//        //删除应用下的用户
//        appsDao.delSytemByUserId(user.getUserId());
//        //删除任务待办
//        bpmdao.delTaskByUserId(user.getUserId());

...
import com.definesys.dsgc.service.ystar.mg.util.ServiceGenerateProxy;
import com.definesys.dsgc.service.ystar.mg.util.Tree;
import com.definesys.dsgc.service.ystar.mg.util.TreeNode;
...
import com.definesys.dsgc.service.ystar.mg.bean.TmplConfigBean;
    ...
    public Response generateServiceCode(String loginUser, TmplConfigBean tcb) throws Exception {
        ...
        if ("0".equals(tcb.getTmplFlag())) {
            if (tcb.getSaForWsdl() != null && tcb.getSaForWsdl().trim().length() > 0) {
                SABean sa = this.muleGenDao.getSaInfoBySaCode(tcb.getSaForWsdl());
...
//        }
    ...
    public List<SvcgenProjInfoBean> queryProjectDir(String uId) {
        ...
        try {
            boolean isAdmin = (uh.isAdmin() || uh.isSuperAdministrator());
            if (isAdmin) {
                list = muleGenDao.queryAllProjectDir();
            } else {
                list = muleGenDao.queryProjectDirByUserId(uId);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        return tmp;
    ...
    public List<ServcieAccountBean> getServcieAccountList(String uid) throws Exception {
        return this.muleGenDao.getServcieAccountList(uid, this.userHelper.user(uid));
    }
    ...
    public List<DeployProfileSltBean> getUserCanDeployList(String uid, String servNo) {
        ...
        if (uh.isSvcGenEditorByServNo(servNo)) {
            return muleGenDao.getUserCanDeployProfiles(uid, servNo);
        }
        return res;
    }
    ...
    public List<DSGCSystemEntities> queryAllSystem() {
        return muleGenDao.queryAllSystem();
    }
    ...
    public Response resolveWsdl(WSDLResolveBean wsdl) throws Exception {
        ...
        if (saCode != null && saCode.trim().length() > 0) {
            SABean sa = this.muleGenDao.getSaInfoBySaCode(saCode);
    ...
    private PageQueryResult<SvcGenObjJsonBean> getSgObjList(boolean isSvcType, SvcGenObjReqBean q, String uid, int pageSize, int pageIndex) {
        UserHelper uh = this.userHelper.user(uid);
        PageQueryResult<SvcGenObjJsonBean> resPageQ = this.muleGenDao.getSgObjList(isSvcType, q, uid, uh, pageSize, pageIndex);
    ...
    public SvcGenObjJsonBean getSvcGenObjInfo(String sgObjCode, String uid, UserHelper uh) {
        SvcGenObjJsonBean res = this.muleGenDao.getSvcGenObjInfo(sgObjCode, uid, uh);
        if (res != null) {
    ...
    public int bindingSgObjToNewServ(SVCCreateBean scc) throws Exception {
        ...
        return this.muleGenDao.bindingSgObjToNewServ(scc, uriList);
    }
    ...
    public int deleteSvcGenObj(String uid, String sgObjCode) throws Exception {
        ...
        return this.muleGenDao.deleteSvcGenObj(objJsonBean, uid, uh);
    }
    ...
    public List<Map<String, String>> getDBConnList(String dbType) {
        ...
        final List<SvcgenConnBean> dbConnList = muleGenDao.getDBConnList(dbType);
    ...
    public void validDBConnInfo(DBconnVO dBconnVO) {
        ...
        if (connectSuccess && StringUtil.isBlank(dBconnVO.getConnId())) {
            ...
            muleGenDao.saveDBConnectInfo(svcgenConnBean);
        }
    }

    public DBconnVO getDBConnDetailByName(String connName) {
        if (StringUtil.isNotBlank(connName)) {
            SvcgenConnBean svcgenConnBean = muleGenDao.getDBConnDetailByName(connName);
    ...
    public  List<Map<String, Object>> queryTableFileds(String tableName, String connectName) {
        ...
        try {
            ...
            if ("oracle".equals(dBconnVO.getDbType())) {
                ...
                sql = "select column_name,(case when find_in_set('YES',  is_nullable) then 'Y' when find_in_set('NO',  is_nullable) then 'N' end) nullable,data_type from INFORMATION_SCHEMA.Columns where upper(table_name)= ? and table_schema= ? ";
            }
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
    ...
    public Boolean checkConnectNameIsExsit(Map<String,String> map){
        if(map != null && !map.isEmpty() && !"".equals(map.get("connectName"))){
            ...
            SvcgenConnBean svcgenConnBean = muleGenDao.getDBConnDetailByName(connectName);
    ...
    public DBDeployProfileBean getDBDeployProfileDetail(String dpId) {
        return this.muleGenDao.getDBDeployProfileDetail(dpId);
    }

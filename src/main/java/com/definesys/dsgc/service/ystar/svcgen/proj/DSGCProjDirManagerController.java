...
public class DSGCProjDirManagerController {
    ...
    public Response pageQuerySysProfileDir(@RequestBody CommonReqBean commonReqBean,
                                           @RequestParam(value = "pageSize", required = false, defaultValue = "10") int pageSize,
                                           @RequestParam(value = "pageIndex", required = false, defaultValue = "1") int pageIndex) {
        PageQueryResult<DSGCSysProfileDir> queryResult = projDirManagerService.pageQuerySysProfileDir(pageIndex, pageSize, commonReqBean);
        return Response.ok().data(queryResult);
    }

    @RequestMapping(value = "/manage/querySysProfileDirList", method = {RequestMethod.POST, RequestMethod.GET})
    public Response querySysProfileDirList(@RequestBody DSGCSysProfileDir sysProfileDir) {
        List<DSGCSysProfileDir> queryResult = projDirManagerService.querySysProfileDirList(sysProfileDir);
        return Response.ok().data(queryResult);
    }
    ...
    public Response saveOrUpdateSysProfileDir(@RequestBody DSGCSysProfileDir sysProfileDirVO) {
        System.out.println(sysProfileDirVO.toString());
        try {
    ...
    @RequestMapping(value = "/manage/querySystemBySubSystem", method = RequestMethod.POST)
    public Response querySystemBySubSystem(@RequestBody String subSystem) {
        JSONObject jsonObject = JSONObject.parseObject(subSystem);
        String subSystemCode = jsonObject.getString("subSystem");
        return Response.ok().data(projDirManagerService.querySystemBySubSystem(subSystemCode));
    }

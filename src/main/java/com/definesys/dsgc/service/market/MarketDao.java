...
//            query.and().eq("type",shareTypNoSpace);
    ...
    public PageQueryResult<MarketEntiy> queryEsb(Map<String, Object> mapVlue, int pageSize, int pageIndex) {
        ...
        MpaasQuery query = sw.buildQuery().sql("select distinct(servId),servNo,servName,servDesc,fromSys,marketStat,marketCategory,creationDate,totalTimes from(\n" +
                "select s.servId,s.servNo,s.servName,s.servDesc,s.fromSys,s.marketStat,s.marketCategory,s.creationDate,sum(s.totalTimes) as totalTimes from (\n" +
                "select t.serv_id as servId,t.serv_no as servNo,t.serv_name as servName,t.serv_desc as servDesc,e.sys_name as fromSys,\n" +
                "t.market_stat as marketStat ,t.market_category as marketCategory,t.creation_date as creationDate,\n" +
                "(case when r.total_times >1 then r.total_times  else 0 end ) as totalTimes \n" +
                "from DSGC_SERVICES t \n" +
                "left join rp_serv_total r on t.serv_no=r.serv_no \n" +
                "left join dsgc_system_entities e on t.subordinate_system=e.sys_code ) s group by s.servNo, s.servId,  s.servName, s.servDesc, s.fromSys, s.marketStat, s.marketCategory, s.creationDate) re");
    ...
    public PageQueryResult<MarketEntiy> queryApi(Map<String, Object> mapVlue, int pageSize, int pageIndex) {
        ...
        String sql = "select distinct(servId),servNo,servName,servDesc,fromSys,marketStat,marketCategory,creationDate,totalTimes from(\n" +
                "select s.servId,s.servNo,s.servName,s.servDesc,s.fromSys,s.marketStat,s.marketCategory,s.creationDate,s.totalTimes as totalTimes from \n" +
                ...
                "   left join (select rp.serv_no,sum(rp.total_times) as total_times from rp_api_year rp group by rp.serv_no )r   on t.api_code=r.serv_no\n" +
                "   left join dsgc_system_entities e   on t.app_code=e.sys_code) s ) re";
        if ("oracle".equals(dbType)) {
            sql = "select distinct(servId),servNo,servName,servDesc,fromSys,marketStat,marketCategory,creationDate,totalTimes from(\n" +
        ...
        MpaasQuery query = sw.buildQuery().sql(sql);
    ...
    public List<Map<String, Object>> getEsbClassType(String sourceType) {
        return sw.buildQuery().sql("select c.cate_code as CATE_CODE,c.cate_name as CATE_NAME,count( CASE WHEN s.market_stat='Y' THEN 1 ELSE NULL END ) TOTAL from dsgc_market_category c left join dsgc_services s on s.market_category=c.cate_code\n" +
    ...
    public List<Map<String, Object>> getApiClassType(String sourceType) {
        return sw.buildQuery().sql("select c.cate_code as CATE_CODE,c.cate_name as CATE_NAME,count( CASE WHEN s.market_stat='Y' THEN 1 ELSE NULL END ) TOTAL from dsgc_market_category c left join dsgc_apis s on s.market_category=c.cate_code\n" +
                "where c.parent_cate=#sourceType \n" +
                "group by c.cate_code,c.cate_name")
                .setVar("sourceType", sourceType)
                .doQuery();

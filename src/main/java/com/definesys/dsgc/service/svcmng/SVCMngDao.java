...
public class SVCMngDao {
    ...
    public PageQueryResult<DSGCService> querySvcMngServList(SVCCommonReqBean param, int pageIndex, int pageSize, String userRole, List<String> sysCodeList, String servIsOnline) {
        //SQLLoader.sql(SqlUtils.SQL_SVC_MNG_LIST);
        String str = "SELECT DS.SERV_NO,DS.SERV_NAME,DS.SERV_STATUS,DSE.SYS_NAME attribue1,DS.SUBORDINATE_SYSTEM,\n" +
                "            DS.SHARE_TYPE,LV.MEANING SHARE_TYPE_MEANING,U.IB_URI, U.HTTP_METHOD,CASE\n" +
                "            WHEN DS.INFO_FULL IS NULL THEN 0  ELSE DS.INFO_FULL END INFO_FULL FROM DSGC_SERVICES DS, DSGC_SERVICES_URI U, DSGC_SYSTEM_ENTITIES DSE,\n" +
                "            (SELECT FV.LOOKUP_CODE,FV.MEANING FROM FND_LOOKUP_TYPES FT, FND_LOOKUP_VALUES FV WHERE FT.LOOKUP_ID = FV.LOOKUP_ID\n" +
                "            AND FT.LOOKUP_TYPE = 'SVC_SHARE_TYPE') LV WHERE DS.SUBORDINATE_SYSTEM = DSE.SYS_CODE AND DS.SERV_NO = U.SERV_NO AND DS.SHARE_TYPE = LV.LOOKUP_CODE\n" +
                "            ";
...
//                .doInsert();
    ...
    public void delSvcPayloadSample(DSGCPayloadSampleBean param) {
        sw.buildQuery()
                .eq("res_code", param.getResCode())
                .eq("req_or_res", param.getReqOrRes())
                .eq("uri_type", param.getUriType())
                .doDelete(DSGCPayloadSampleBean.class);
    }
    ...
    public List<Map<String, Object>> getUrlByServNo(String servNo) {
        return sw.buildQuery().sql("select t.ib_uri from DSGC_SERVICES_URI t where t.serv_no=#servNo")
    ...
    public void modifyServStatus(Map<String, String> map) {
        String servEnable = "Y";
        if (map.get("label").equals("1")) {
            servEnable = "Y";
        } else {
            servEnable = "N";
        }
        sw.buildQuery().update("serv_status", map.get("label"))
                .update("SERV_ENABLE", servEnable)
                .eq("serv_no", map.get("servNo")).doUpdate(DSGCService.class);
    }

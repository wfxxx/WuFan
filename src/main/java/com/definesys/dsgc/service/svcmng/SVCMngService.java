...
//        svcMngInfoListBean.setDeplEnv(this.svcMngDao.getDeployEnvInfo(dsgcService.getIbUri()));
    ...
    public SVCServBasicInfoDTO queryBasicInfoByServNo(String servNo) {
        ...
        dto.setServEnable(dsgcService.getServEnable());
    ...
    public Boolean saveServBasicInfo(SVCServBasicInfoDTO svcServBasicInfo) {
        DSGCService servByServNo = svcMngDao.queryServByServNo(svcServBasicInfo.getServNo());
        if (servByServNo != null) {
            ...
            dsgcService.setServEnable(svcServBasicInfo.getServEnable());
            svcMngDao.saveServBasicInfo(dsgcService);
            completionThread(dsgcService.getServNo(), svcServBasicInfo.getType());
            if (servByServNo.getServEnable() == null || !servByServNo.getServEnable().equals(svcServBasicInfo.getServEnable())) {
                return true;
            } else {
                return false;
            }
...
//            }
    ...
    public void saveParamData(SaveParamDataVO param) {
        String payloadType = param.getPayloadType();
        String svcCode = param.getServNo();
        String paramType = param.getParamType();
        String uriType = "json".equals(payloadType) ? "REST" : ("xml".equals(payloadType) ? "SOAP" : ("x-www-form-urlencoded".equals(payloadType) ? "FORM" : null));
        if ("none".equals(payloadType)) {
            svcMngDao.delSvcPayloadSample(new DSGCPayloadSampleBean(svcCode, paramType, null));
            svcMngDao.delServPayloadParam(new DSGCPayloadParamsBean(svcCode, paramType));
        } else {
            ...
            if (StringUtil.isNotBlank(param.getRestSample())) {
                DSGCPayloadSampleBean dsgcPayloadSampleBean = new DSGCPayloadSampleBean(svcCode, paramType, "REST");
    ...
    public List<Map<String, String>> resloveXML(String XMLDemo) {
        ...
        while (m.find()) {
            ...
            //去除Body和Header标签 20201123
            if (node.toLowerCase().contains("body") || node.toLowerCase().equals("header")) {
                continue;
            }

...
public class ApiHomeDao {
    ...
    public ApiHomeHisto queryTrafficCostNow() {
        String sql = "select (case when avg(t.req_msg_size)is null then 0 else avg(t.req_msg_size) end )  as VALUE from DAG_LOG_INSTANCE t where CURRENT_TIMESTAMP >str_to_date(DATE_FORMAT(CURRENT_TIMESTAMP,'%Y-%m-%d %H'),'%Y-%m-%d %H') order by t.creation_date";

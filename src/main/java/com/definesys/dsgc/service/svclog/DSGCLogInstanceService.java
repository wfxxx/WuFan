...
public class DSGCLogInstanceService {
    ...
    public PageQueryResult<DSGCLogInstance> query(List<Map<String, String>> keyword, String userRole, String uid, LogInstanceQueryDTO instance, int pageSize, int pageIndex) {
        try {
...
//    }
    ...
    public String dealPath(String startTime, String servNo, String ibLob) {
        String folderName = StringUtils.substringBefore(startTime.trim().replace("-", ""), " ");
        String folderName1 = folderName.substring(0, 6);
        String path = dsgcLogInstanceDao.findFndPropertiesByKey("LOG_PL_STORE_ROOT_DIR").getPropertyValue() + "/" + folderName1 + "/" + folderName + "/" + servNo + "/" + ibLob;
    ...
    public Response queryEsbServLogInst(TempQueryLogCondition tempQueryLogCondition, int pageSize, int pageIndex, String userRole, String userId, HttpServletRequest request) throws Exception {
        ...
        List<Map<String, String>> keyword = tempQueryLogCondition.getKeywordForm();

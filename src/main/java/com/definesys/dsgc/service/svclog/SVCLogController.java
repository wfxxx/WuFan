...
public class SVCLogController {
    ...
    public Response deleteResultObj(@RequestBody DSGCValidResult validResut,
                                    HttpServletRequest request) {
        String userRole = request.getHeader("userRole");
        String userId = request.getHeader("uid");
    ...
    public Response addResultObj(@RequestBody DSGCValidResult validResult,
                                    HttpServletRequest request) {
        String userRole = request.getHeader("userRole");
        String userId = request.getHeader("uid");
        int result =  this.sls.addResultObj(validResult,userRole,userId);
        if (result == -1){
            return Response.error("无权限操作");
        }else {
            return Response.ok();

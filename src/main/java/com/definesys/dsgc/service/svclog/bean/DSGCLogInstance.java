...
@SQLQuery(value = {
        ...
        @SQL(view = "Log_Instance_view",sql = " SELECT LOGS.* FROM ( SELECT track_id FROM dsgc_log_bizkey biz WHERE biz.bus_serv_no = #servNo and (biz.Col1 = #COL1 OR biz.Col2 = #COL2 OR biz.Col3 = #COL3 OR biz.Col4 = #COL4 OR biz.Col5 = #COL5 OR biz.Col6 = #COL6 OR biz.Col7 = #COL7 OR biz.Col8 = #COL8 OR biz.Col9 = #COL9 OR biz.Col10 = #COLTEN)) t_id left join (select insv.*, bpv.payload_data from dsgc_log_instance_v insv, (select pv.track_id, GROUP_CONCAT(pv.payload_data SEPARATOR '<======报文结束======>') payload_data from dsgc_log_body_payload pv group by pv.track_id) bpv where insv.track_id = bpv.track_id) logs on logs.track_id = t_id.track_id"),
...
public class DSGCLogInstance {
    ...
    private Integer busCount;//ystar 20210507

...
//        sysCode = dsgcSystemUser.getSysCode();
    ...
    public PageQueryResult<SVCLogResultResDTO> querySvcLogResultByCon(SVCLogQueryBean q, int pageSize, int pageIndex, String userRole, String userId) {
        if ("Tourist".equals(userRole)) {
            ...
            PageQueryResult<DSGCValidResult> result = sldao.querySvcLogResultByCon(q, pageSize, pageIndex, userRole, sysCodeList);
            List<SVCLogResultResDTO> servResDTOs = new ArrayList<>();
            for (DSGCValidResult item : result.getResult()) {
                ...
                svcLogResultResDTO.setWarningValue(item.getWarningValue());
                svcLogResultResDTO.setMsgValue(item.getMsgValue());
    ...
    @Transactional(rollbackFor = Exception.class)
    public int deleteResultObj(DSGCValidResult validResut, String userRole, String userId) {
        if ("Tourist".equals(userRole)) {
            return -1;
        }
        Boolean temp = true;
    ...
    @Transactional(rollbackFor = Exception.class)
    public int addResultObj(DSGCValidResult validResult, String userRole, String userId) {
        if ("Tourist".equals(userRole)) {
            return -1;
        }
        Boolean temp = true;
        if ("SystemLeader".equals(userRole)) {
            List<String> userSubsystemList = this.queryUserSubSystem(userId);
            DSGCService dsgcService = this.sldao.queryServiceByServNo(validResult.getServNo());
            for (int i = 0; i < userSubsystemList.size(); i++) {
                if (dsgcService.getSubordinateSystem().equals(userSubsystemList.get(i))) {
                    temp = true;
                    break;
        ...
        if (!temp) {
            ...
            this.sldao.addResultObj(validResult);
            return 1;
        }
    }
